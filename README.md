...
Claude Code 是目前使用最广泛的 AI 编程 Agent。它能理解整个代码仓库、自主执行多步编程任务、安全地运行命令——而这一切背后是 **50 万行 TypeScript 源码**中沉淀的工程智慧。

Anthropic 开源了这份源码。**但 50 万行代码，从哪里开始读？**

本项目就是答案。我们从源码中提炼出 **11 篇专题文档**，覆盖了从核心循环到安全防护的每一个关键设计决策。不管你是想造自己的 AI Agent，还是想更深入地理解和使用 Claude Code，这里都是最短路径。

## 相关项目

- **[claude-code-from-scratch](https://github.com/Windy3f3f3f3f/claude-code-from-scratch)** — 从零实现 Claude Code 核心功能的最小化实现与分步教程

## 这份源码为什么值得深入研究？

大多数 AI Agent 框架都是"demo 级别"——跑通一个场景就宣布成功。Claude Code 不同，它是一个**日活百万级开发者实际使用的生产系统**，需要处理的问题远比 demo 复杂：

- 对话动辄上万 token，上下文窗口不够用怎么办？
- 用户让 AI 执行 `rm -rf /`，怎么拦住？
- 66 个内置工具同时存在，怎么协调？
- 网络断了、API 过载了、token 超限了，怎么不崩？
- 怎么让用户感觉"快"，哪怕模型推理本身就要几十秒？
