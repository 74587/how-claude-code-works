...
先建立直觉。服务端的前缀缓存**不是 diff，而是 hash**——它不检查"这次的前缀和上次有多像"，而是检查"是否完全一样"。一个 100K token 的前缀，哪怕只改了一个字符，对缓存来说就是全新的前缀，需要重新处理全部 100K token。

而且，构成"缓存键"的不只是提示词文本本身。以下任何一项变化都会导致缓存失效：

- **内容变化**：系统提示词文字、工具 schema 定义
- **元数据变化**：缓存作用域（scope）、过期时间（TTL）、API 请求中的 beta header
- **顺序变化**：工具数组的排列顺序

这意味着缓存的"攻击面"比大多数人想象的要大得多。Claude Code 为此建立了**四层防御**，每一层解决一类失效风险。
...
> **设计权衡**：锁存牺牲了灵活性（无法中途关闭某个 beta 功能、无法中途降级 TTL），换来了缓存稳定性。在 50-100K token 缓存失效的高成本面前，这是值得的取舍。

### 第三层：工具数组的缓存稳定性

工具 schema 数组中，**最后一个常规工具**被标记了 `cache_control`，作为缓存断点——服务端会缓存到这个位置为止的所有内容（系统提示词 + 工具定义）。

基于这个断点，Claude Code 做了一个巧妙的排列：可选的服务端工具（如 advisor）被放在断点**之后**。这意味着用户开启或关闭 `/advisor`，只会改变断点之后的一小段内容，而断点之前已缓存的大量系统提示词和工具定义完全不受影响。

此外，MCP 工具通过**延迟加载**（`defer_loading`）机制，在 Tool Search 被调用之前不会出现在工具数组中——这与第一层的分割策略配合，尽量减少因用户特有工具导致的缓存差异。
...
> **小结**：四层防御各有分工——分割决定什么**能**被缓存，锁存确保它**持续**被缓存，工具排序**最小化**变更影响，检测系统**验证**一切正常运转。
